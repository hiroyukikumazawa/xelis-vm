...
impl Stack {
    ...
    #[inline]
    pub fn push_stack(&mut self, value: ValuePointer) -> Result<(), VMError> {
    ...
    #[inline]
    pub fn get_stack_at(&self, index: usize) -> Result<&ValuePointer, VMError> {
        self.stack.get(index).ok_or(VMError::StackIndexOutOfBounds)
    }

    // Push a value to the stack without checking the stack size
    #[inline(always)]
    pub fn push_stack_unchecked(&mut self, value: ValuePointer) {
    ...
    #[inline]
    pub fn extend_stack<I: IntoIterator<Item = ValuePointer> + ExactSizeIterator>(&mut self, values: I) -> Result<(), VMError> {
    ...
    #[inline]
    pub fn pop_stack(&mut self) -> Result<ValuePointer, VMError> {
    ...
    // Get the last value from the stack
    #[inline]
    pub fn last_stack(&self) -> Result<&ValuePointer, VMError> {
        self.stack.last().ok_or(VMError::EmptyStack)
    }

    // Get the last mutable value from the stack
    #[inline]
    pub fn last_mut_stack(&mut self) -> Result<&mut ValuePointer, VMError> {
        self.stack.last_mut().ok_or(VMError::EmptyStack)
    }

    // Get the inner stack
    #[inline]
    pub fn get_inner(&mut self) -> &mut Vec<ValuePointer> {
        &mut self.stack
    }
