...
pub type Handler<'a> = (fn(&Backend<'a>, &mut Stack, &mut ChunkManager<'a>, &mut Context<'a>) -> Result<InstructionResult, VMError>, u64);
...
impl<'a> InstructionTable<'a> {
    ...
    pub fn execute(&self, opcode: u8, backend: &Backend<'a>, stack: &mut Stack, chunk_manager: &mut ChunkManager<'a>, context: &mut Context<'a>) -> Result<InstructionResult, VMError> {
...
fn unimplemented<'a>(_: &Backend<'a>, _: &mut Stack, _: &mut ChunkManager<'a>, _: &mut Context<'a>) -> Result<InstructionResult, VMError> {
    Err(VMError::InvalidOpCode)
}

fn return_fn<'a>(_: &Backend<'a>, _: &mut Stack, _: &mut ChunkManager<'a>, _: &mut Context<'a>) -> Result<InstructionResult, VMError> {
    Ok(InstructionResult::Break)
}

fn jump<'a>(_: &Backend<'a>, _: &mut Stack, manager: &mut ChunkManager<'a>, _: &mut Context<'a>) -> Result<InstructionResult, VMError> {
    let addr = manager.read_u32()?;
    manager.set_index(addr as usize)?;
    Ok(InstructionResult::Nothing)
}

fn jump_if_false<'a>(_: &Backend<'a>, stack: &mut Stack, manager: &mut ChunkManager<'a>, _: &mut Context<'a>) -> Result<InstructionResult, VMError> {
    let addr = manager.read_u32()?;
    let value = stack.pop_stack()?;
    if !value.handle().as_bool()? {
        manager.set_index(addr as usize)?;
    }
    Ok(InstructionResult::Nothing)
}